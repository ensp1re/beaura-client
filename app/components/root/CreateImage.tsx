...
import { FaCopy, FaSpinner, FaUpload } from "react-icons/fa"
import { ChangeEvent, useEffect, useRef, useState } from "react"
import { IAspectRatioOption, ITransformationData, ITransformationUpload } from "@/interfaces/root.interface"
...
export default function ImageTransformer() {
    ...
    const [transformedData, setTransformedData] = useState<ITransformationData | null>(null);
    ...
    const handleSubmission = async (): Promise<void> => {
        ...
        try {
            ...
            useToast.success(response.message as string);
        } catch (error: unknown) {
            if (error instanceof Error) {
        ...
        <div className="container mx-auto p-4 lg:p-8">
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <Card className="relative aspect-[3/4] w-full overflow-hidden bg-gray-100">
                    ...
                    {
                        transformedData && transformedData?.toImage && (
                            <Image
                                src={transformedData?.toImage}
                                alt={`Transformed Image | ${transformedData.title?.trim()}`}
                                fill
                                className="object-cover"
                                sizes="(max-width: 768px) 100vw, (max-width: 1200px) 50vw, 33vw"
                            />
...

                    {
                        isDataLoading && (
                            <div className="absolute  inset-0 flex items-center flex-col gap-2 justify-center text-muted-foreground">
                                <FaSpinner className="animate-spin h-8 w-8 mr-2" />
                                Transforming...
                            </div>
                        )
                    }
                ...
                <Card className="p-6 space-y-6">
                    ...
                    <Button
                        ...
                        className={`w-full mt-4 ${!selectedFile || !data.title || !data.prompt ? "cursor-not-allowed" : "bg-gray-500 hover:bg-gray-600"}`}
                    >
                        Reset
                    </Button>
                    <Button
                        onClick={handleSubmission}
                        disabled={!selectedFile || !data.title || !data.prompt || isDataLoading}
